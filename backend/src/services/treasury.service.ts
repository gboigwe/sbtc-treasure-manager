...
export class TreasuryService {
  ...
  async getBusinessByWallet(walletAddress: string) {
  ...
  async processPayment(data: {
    ...
    try {
      const contractResult = await this.stacksService.createPayment(
        payment.business.walletAddress,
        Math.floor(data.amount * 1000000), // Convert to microunits
  ...
  async rebalanceTreasury(businessId: string) {
  ...
  async createYieldStrategy(businessId: string, data: {
    protocol: string;
    allocation: number;
    currentAPY: number;
  }) {
